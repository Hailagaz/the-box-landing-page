...
document.addEventListener("DOMContentLoaded",()=>{for(const e of document.querySelectorAll(".projects__btn"))e.addEventListener("click",e=>{var t,o=e.target.dataset.index,s=document.querySelector(".projects__block.active");s.dataset.index!==o&&(t=document.querySelector(".projects__btn.active"),e.target.classList.add("active"),t.classList.remove("active"),e=document.querySelector(`.projects__block[data-index="${o}"]`),s.classList.remove("active"),e.classList.add("active"))});fetch("js/projectsCommercial.json").then(e=>e.json()).then(e=>{let t="";for(var o of e)t+=`
						<div class="swiper-slide projects__card">
							<img class="projects__image" src="${o.photo}"
